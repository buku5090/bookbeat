...
function formatDateLong(dt) {
  const d = new Date(dt);
  return d.toLocaleString("ro-RO", { weekday: "long", day: "2-digit", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" });
}
      ...
      <section className="mx-auto max-w-4xl px-4 sm:px-6 py-10">
        <div className="overflow-hidden rounded-3xl border border-white/10 bg-white/5">
          ...
          <div className="p-6">
            ...
            <p className="mt-2 text-white/80">{formatDateLong(event.date)} • {event.venue}</p>
            <p className="mt-4 text-white/90">{event.description}</p>
            <div className="mt-6 flex items-center justify-between">
              <span className="text-xl font-semibold">{event.price}</span>
              <Button className="bg-white text-black hover:opacity-90">Rezervă / Bilete</Button>
            </div>
          </div>
        </div>
      </section>
